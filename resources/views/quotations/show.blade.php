...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-500">Created on {{ $quotation->created_at->format('d/m/Y') }}</p>
        </div>
        <div class="flex gap-2">
            @if(!$quotation->converted_at)
            <button onclick="convertQuotation({{ $quotation->id }})" class="btn-gold px-4 py-2 rounded-lg text-sm font-semibold">Convert to Invoice</button>
            @endif
            ...
            <a href="{{ route('quotations.index') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Back</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border p-6">
        <div class="flex items-start justify-between mb-6 pb-6 border-b">
            ...
            <div class="text-right">
                ...
                <p class="text-xs text-gray-500">Date: {{ $quotation->created_at->format('d/m/Y') }}</p>
                @if($quotation->valid_until)
                <p class="text-xs text-gray-500">Valid Until: {{ $quotation->valid_until->format('d/m/Y') }}</p>
                @endif
            </div>
        </div>

        <div class="mb-6">
            <p class="text-[10px] font-semibold text-gray-500 uppercase mb-1">Bill To</p>
            <p class="text-sm font-semibold text-gray-900">{{ $quotation->customer?->name ?? 'Walk-in Customer' }}</p>
        ...
        <table class="w-full mb-6">
            <thead>
                <tr class="border-b bg-gray-50">
                    <th class="text-left px-4 py-2 text-[10px] font-semibold text-gray-500 uppercase">Item</th>
                    <th class="text-center px-4 py-2 text-[10px] font-semibold text-gray-500 uppercase">Qty</th>
                    <th class="text-right px-4 py-2 text-[10px] font-semibold text-gray-500 uppercase">Price</th>
                    <th class="text-right px-4 py-2 text-[10px] font-semibold text-gray-500 uppercase">Total</th>
                </tr>
            </thead>
            <tbody>
...
async function convertQuotation(id) {
    saConfirm({
        title: 'Convert to Invoice?', text: 'This quotation will be converted to an invoice.', icon: 'warning',
        confirmText: 'Yes, Convert', confirmColor: 'emerald',
        onConfirm: async () => {
