...
<div class="max-w-md mx-auto bg-white rounded-2xl shadow-cardlg p-8 mt-6">
    <div class="text-center border-b-2 border-dashed border-gray-200 pb-4">
        ...
        <p class="text-xs font-bold text-gray-600 mt-1">Risiti: {{ $sale->receipt_no }}</p>
        <p class="text-xs text-gray-400">Muuzaji: {{ $sale->user->name ?? 'N/A' }}</p>
    </div>
    ...
    <div class="py-4 border-b-2 border-dashed border-gray-200">
        ...
        <div class="flex justify-between text-sm py-1">
            <div>
                ...
                <span class="text-gray-400 text-xs block">TZS {{ number_format($item->price, 0) }} kwa kila</span>
            </div>
    ...
    <div class="py-3 space-y-1.5 border-b-2 border-dashed border-gray-200">
        <div class="flex justify-between text-sm font-bold text-gray-500"><span>Jumla:</span><span>TZS {{ number_format($sale->subtotal, 0) }}</span></div>
        @if($sale->discount > 0)
        <div class="flex justify-between text-sm font-bold text-red-500"><span>Punguzo:</span><span>-TZS {{ number_format($sale->discount, 0) }}</span></div>
        @endif
        @if($sale->vat > 0)
        <div class="flex justify-between text-sm font-bold text-gray-500"><span>VAT:</span><span>TZS {{ number_format($sale->vat, 0) }}</span></div>
        @endif
        <div class="flex justify-between text-lg font-black text-emerald-700"><span>JUMLA:</span><span>TZS {{ number_format($sale->total, 0) }}</span></div>
        <div class="flex justify-between text-sm font-bold text-gray-500"><span>Malipo ({{ ucfirst(str_replace('_', ' ', $sale->payment_method)) }}):</span><span>TZS {{ number_format($sale->paid, 0) }}</span></div>
        @if($sale->change > 0)
    ...
    @if($sale->customer)
    <div class="py-2 text-center text-xs font-bold text-gray-500">Mteja: {{ $sale->customer->name }}</div>
    @endif

    <div class="text-center pt-4">
        <p class="text-xs font-bold text-gray-400">Asante kwa kutumia Wazabiashara! 🇹🇿</p>
        <p class="text-[10px] text-gray-300 mt-1">Biashara Yako, Mkononi Mwako</p>
    </div>

    <button onclick="window.print()" class="mt-4 w-full btn-gold font-black py-3 rounded-xl">🖨️ Chapisha</button>
</div>
