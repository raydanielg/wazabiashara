...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Simamia matawi ya biashara yako</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Jumla ya Matawi</p><p class="text-xl font-bold text-gray-800">{{ $totalBranches }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Matawi Hai</p><p class="text-xl font-bold text-emerald-600">{{ $activeBranches }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Wafanyakazi</p><p class="text-xl font-bold text-gray-800">{{ $totalUsers }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Mauzo Jumla</p><p class="text-xl font-bold text-gray-800">{{ $totalSales }}</p></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" id="branchesGrid">
        ...
        <div class="bg-white rounded-2xl border p-5 shadow-sm hover:shadow-md transition-all {{ session('active_branch_id') == $branch->id ? 'border-emerald-300' : 'border-gray-200' }}" id="branch-card-{{ $branch->id }}">
            <div class="flex items-start justify-between">
                ...
                <span class="px-2.5 py-1 rounded-full text-xs font-bold {{ $branch->status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500' }}">{{ $branch->status === 'active' ? 'Hai' : 'Imezimwa' }}</span>
            </div>
            ...
            <div class="mt-4 flex gap-2">
                <form method="POST" action="{{ route('branches.switch', $branch) }}">@csrf<button class="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 text-xs font-bold transition-all">Badilisha</button></form>
                <button onclick="editBranch({{ $branch->id }}, '{{ addslashes($branch->name) }}', '{{ addslashes($branch->location ?? '') }}', '{{ $branch->phone ?? '' }}', '{{ $branch->status }}')" class="w-8 h-8 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-500 transition-all flex items-center justify-center" title="Hariri"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
        ...
        <div class="col-span-full text-center py-16">
            ...
            <p class="text-gray-400 font-medium text-sm">Hakuna matawi. Ongeza tawi la kwanza!</p>
        </div>
        @endforelse
    </div>
</div>
...
<div id="branchModal" class="fixed top-0 right-0 bottom-0 w-full sm:w-[420px] bg-white z-50 transform translate-x-full transition-transform duration-300 ease-out overflow-y-auto flex flex-col">
    ...
    <form id="branchForm" class="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Jina la Tawi *</label>
        ...
        <div id="statusField" class="hidden">
            ...
            <select name="status" id="branchStatus" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="active">Hai</option><option value="inactive">Imezimwa</option></select>
        </div>
    </form>
...
function openBranchModal() {
    ...
    branchEditId = null;
    document.getElementById('branchModalTitle').innerHTML = '<div class="w-8 h-8 rounded-xl bg-emerald-50 grid place-items-center"><svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg></div> Tawi Jipya';
...
function deleteBranch(id) {
    Swal.fire({
        title: 'Una uhakika?', text: 'Tawi hili litafutwa kabisa.',
