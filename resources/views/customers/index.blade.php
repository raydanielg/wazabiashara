...
@section('page_title', 'Customers & Debts')
...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Manage customers and their debts</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Total Customers</p><p class="text-xl font-bold text-gray-800">{{ $totalCustomers }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Active Customers</p><p class="text-xl font-bold text-emerald-600">{{ $activeCustomers }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">With Debts</p><p class="text-xl font-bold text-red-600">{{ $customersWithDebt }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Total Debt</p><p class="text-xl font-bold text-gold-600">TZS {{ number_format($totalDebt, 0) }}</p></div>
            </div>
        </div>
    </div>
    ...
    <form method="GET" class="flex gap-2 sm:gap-3 flex-wrap">
        <div class="relative flex-1 min-w-[180px]">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search customer..." class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium transition-all">
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-500 font-semibold text-xs uppercase tracking-wide">
                    <tr><th class="px-4 py-3 text-left">Name</th><th class="px-4 py-3 text-left hidden sm:table-cell">Phone</th><th class="px-4 py-3 text-right">Credit Limit</th><th class="px-4 py-3 text-right">Current Debt</th><th class="px-4 py-3 text-center">Status</th><th class="px-4 py-3 text-center">Actions</th></tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                @if($c->current_debt > 0)<a href="{{ route('customers.debts', $c) }}" class="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition-all flex items-center justify-center" title="Debts"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1"/></svg></a>@endif
                                <button onclick="editCust({{ $c->id }}, '{{ addslashes($c->name) }}', '{{ $c->phone ?? '' }}', '{{ $c->email ?? '' }}', '{{ addslashes($c->address ?? '') }}', {{ $c->credit_limit }}, '{{ $c->status }}')" class="w-8 h-8 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 transition-all flex items-center justify-center" title="Edit"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                    ...
                    <tr><td colspan="6" class="px-4 py-16 text-center">
                        ...
                        <p class="text-gray-400 font-medium text-sm">No customers found.</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="custModal" class="fixed top-0 right-0 bottom-0 w-full sm:w-[420px] bg-white z-50 transform translate-x-full transition-transform duration-300 ease-out overflow-y-auto flex flex-col">
    ...
    <form id="custForm" class="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div id="custStatusField" class="hidden">
            ...
            <select name="status" id="custStatus" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="active">Active</option><option value="inactive">Inactive</option></select>
        </div>
    </form>
...
function openCustModal() {
    custEditMode = false;
    custEditId = null;
    document.getElementById('custModalTitle').innerHTML = '<div class="w-8 h-8 rounded-xl bg-emerald-50 grid place-items-center"><svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg></div> New Customer';
