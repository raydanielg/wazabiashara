...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-xl border p-4"><div class="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center mb-2"><svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 3v1a1 1 0 001 1h3a1 1 0 001-1V3m0 0V2a1 1 0 00-1-1H5a1 1 0 00-1 1v1m4 0h.01M7 3h.01M20 3v1a1 1 0 01-1 1h-3a1 1 0 01-1-1V3m0 0V2a1 1 0 011-1h5a1 1 0 011 1v1m-4 0h.01M17 3h.01M7 8h10M7 12h10M7 16h6"/></svg></div><p class="text-lg font-bold text-gray-900">{{ number_format($totalCards) }}</p><p class="text-[10px] text-gray-500">Kadi Jumla</p></div>
    ...
    <div class="flex items-center justify-between">
        <div><h2 class="text-lg font-bold text-gray-900">Kadi za Salamu</h2><p class="text-xs text-gray-500">Tuma kadi za salamu kwa wateja wako</p></div>
        <button onclick="openGreetingModal()" class="btn-gold px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Tengeneza Kadi</button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div class="p-4">
                ...
                @if($card->customer)<p class="text-xs text-gray-500 mt-1">Kwa: {{ $card->customer->name }}</p>@endif
                <p class="text-xs text-gray-600 mt-2 line-clamp-2">{{ $card->message }}</p>
                <div class="flex items-center gap-2 mt-3 pt-3 border-t">
                    <a href="{{ route('cards.greeting.share', $card->share_token) }}" target="_blank" class="flex-1 text-center px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-xs font-semibold hover:bg-emerald-100">Shiriki</a>
        ...
        @empty<div class="col-span-full text-center py-12 text-sm text-gray-400">Hakuna kadi bado. Bonyeza "Tengeneza Kadi" kuanza.</div>@endforelse
    </div>
    ...
    <div class="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl overflow-y-auto">
        ...
        <div class="sticky top-0 bg-gradient-to-r from-emerald-600 to-emerald-800 px-6 py-5 flex items-center justify-between z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-white">Tengeneza Kadi ya Salamu</h3>
                    <p class="text-[10px] text-emerald-200">Jaza taarifa za kadi</p>
                </div>
            </div>
        ...
        <form id="greetingForm" class="p-6 space-y-5" enctype="multipart/form-data">
            ...
            {{-- Type --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Aina ya Kadi</label>
                <div class="relative">
            ...
            {{-- Title --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Kichwa cha Kadi</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    ...
                    <input type="text" name="title" required placeholder="Mfano: Hongera kwa Kuazaliwa!" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm">
                </div>
            </div>
            {{-- Message --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Ujumbe</label>
                <div class="relative">
                    ...
                    <textarea name="message" rows="4" placeholder="Andika ujumbe wako hapa..." class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm resize-none"></textarea>
                </div>
            </div>
            {{-- Customer --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Mteja</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    ...
                    <select name="customer_id" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm appearance-none bg-white">
                        <option value="">-- Chagua Mteja (Hiari) --</option>
            ...
            {{-- Image --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Picha ya Kadi</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="button" onclick="closeGreetingModal()" class="px-5 py-3 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Funga</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('greetingForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (result.success) { Toastify({ text: result.message, duration: 3000, gravity: 'bottom', position: 'right', style: { background: '#024938' } }).showToast(); closeGreetingModal(); setTimeout(() => location.reload(), 800); }
