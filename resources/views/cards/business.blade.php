...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div><h2 class="text-lg font-bold text-gray-900">Business Cards</h2><p class="text-xs text-gray-500">Create and share business cards</p></div>
        <button onclick="openBusinessCardModal()" class="btn-gold px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Create Card</button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div class="flex items-center gap-2 px-4 pb-4">
                <a href="{{ route('cards.business.share', $card->share_token) }}" target="_blank" class="flex-1 text-center px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-xs font-semibold hover:bg-emerald-100">Share</a>
        ...
        @empty<div class="col-span-full text-center py-12 text-sm text-gray-400">No business cards yet.</div>@endforelse
    </div>
</div>
    ...
    <div class="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl overflow-y-auto">
        ...
        <div class="sticky top-0 bg-gradient-to-r from-emerald-600 to-emerald-800 px-6 py-5 flex items-center justify-between z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-white">Create Business Card</h3>
                    <p class="text-[10px] text-emerald-200">Fill in your card details</p>
                </div>
            </div>
        ...
        <form id="businessCardForm" class="p-6 space-y-5" enctype="multipart/form-data">
            ...
            {{-- Card Name --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Card Name</label>
                <div class="relative">
                    ...
                    <input type="text" name="card_name" required placeholder="e.g. Sales Card" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm">
                </div>
            </div>
            {{-- Owner Name --}}
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="owner_name" required placeholder="Full name" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm">
                </div>
            </div>
            {{-- Phone --}}
            <div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" placeholder="email@business.co.tz" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm">
                </div>
            </div>
            {{-- Address --}}
            <div>
                ...
                <div class="relative">
                    ...
                    <textarea name="address" rows="2" placeholder="Business address..." class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm resize-none"></textarea>
                </div>
            </div>
            {{-- Website --}}
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="website" placeholder="www.business.co.tz" class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm">
                </div>
            </div>
            {{-- Logo --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Business Logo</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="social_instagram" placeholder="@business" class="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-xs">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="social_twitter" placeholder="@business" class="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-xs">
                        </div>
                    </div>
                    <div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="button" onclick="closeBusinessCardModal()" class="px-5 py-3 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Close</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('businessCardForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (result.success) { showToast('success', 'Success', result.message); closeBusinessCardModal(); setTimeout(() => location.reload(), 800); }
    } catch (err) { showToast('error', 'Error', 'An error occurred'); }
});

async function deleteBusinessCard(id) {
    saConfirm({ title: 'Delete Card?', text: 'Are you sure you want to delete this card?', icon: 'danger', confirmText: 'Yes, Delete', confirmColor: 'red', onConfirm: async () => { const res = await fetch(`/cards/business/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' } }); const result = await res.json(); if (result.success) { showToast('success', 'Success', result.message); setTimeout(() => location.reload(), 800); } } });
}
