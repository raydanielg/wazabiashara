...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Rekodi manunuzi ya bidhaa kutoka kwa wasambazaji</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Manunuzi Jumla</p><p class="text-xl font-bold text-gray-800">{{ $totalPurchases }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Mwezi Huu</p><p class="text-xl font-bold text-gold-600">{{ $monthPurchases }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Thamani Mwezi</p><p class="text-xl font-bold text-emerald-600">TZS {{ number_format($monthTotal, 0) }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Deni (Credit)</p><p class="text-xl font-bold text-red-600">{{ $creditPurchases }}</p></div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
        ...
        <form method="POST" action="{{ route('suppliers.purchases.store') }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                <div><label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Tawi *</label><select name="branch_id" required class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all">@foreach($branches as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach</select></div>
                <div><label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Msambazaji</label><select name="supplier_id" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="">— Nje —</option>@foreach($suppliers as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach</select></div>
                <div><label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Malipo</label><select name="payment_status" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="paid">Lipa</option><option value="credit">Deni</option><option value="partial">Sehemu</option></select></div>
            </div>
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
                <span class="font-bold text-lg text-emerald-600">Jumla: <span id="purchaseTotal">TZS 0</span></span>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-500 font-semibold text-xs uppercase tracking-wide">
                    <tr><th class="px-4 py-3 text-left">Ref</th><th class="px-4 py-3 text-left hidden sm:table-cell">Tawi</th><th class="px-4 py-3 text-left hidden md:table-cell">Msambazaji</th><th class="px-4 py-3 text-right">Jumla</th><th class="px-4 py-3 text-center">Malipo</th><th class="px-4 py-3 text-center hidden sm:table-cell">Status</th><th class="px-4 py-3 text-left hidden sm:table-cell">Tarehe</th></tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-600 hidden md:table-cell">{{ $p->supplier?->name ?? 'Nje' }}</td>
                    ...
                    <tr><td colspan="7" class="px-4 py-16 text-center">
                        ...
                        <p class="text-gray-400 font-medium text-sm">Hakuna manunuzi.</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
function addPurchaseRow() {
    ...
    const html = `<div class="flex gap-2 items-center" id="prow${rowIdx}">
        <select name="items[${rowIdx}][product_id]" required class="flex-1 px-3 py-2 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white transition-all" onchange="updateRowTotal(this)"><option value="">— Chagua —</option>${opts}</select>
        ...
        <input type="number" name="items[${rowIdx}][cost_price]" placeholder="Bei" min="0" step="0.01" required class="w-24 px-2 py-2 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-bold" oninput="updateRowTotal(this)">
