...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Rekodi matumizi ya biashara</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Leo</p><p class="text-xl font-bold text-gray-800">TZS {{ number_format($todayExpenses, 0) }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Mwezi Huu</p><p class="text-xl font-bold text-gold-600">TZS {{ number_format($monthExpenses, 0) }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Jumla</p><p class="text-xl font-bold text-red-600">TZS {{ number_format($totalExpenses, 0) }}</p></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-xs text-gray-500 font-semibold">Kategoria Mwezi</p><p class="text-xl font-bold text-gray-800">{{ $expensesByCategory->count() }}</p></div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
        <h3 class="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2">
            ...
            Matumizi kwa Kategoria (Mwezi Huu)
        </h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-500 font-semibold text-xs uppercase tracking-wide">
                    <tr><th class="px-4 py-3 text-left">Kategoria</th><th class="px-4 py-3 text-left hidden sm:table-cell">Maelezo</th><th class="px-4 py-3 text-left hidden md:table-cell">Tawi</th><th class="px-4 py-3 text-right">Kiasi</th><th class="px-4 py-3 text-left hidden sm:table-cell">Tarehe</th><th class="px-4 py-3 text-center">Kitendo</th></tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors" id="exp-row-{{ $e->id }}">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="editExp({{ $e->id }})" class="w-8 h-8 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 transition-all flex items-center justify-center" title="Hariri"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                    ...
                    <tr><td colspan="6" class="px-4 py-16 text-center">
                        ...
                        <p class="text-gray-400 font-medium text-sm">Hakuna matumizi.</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="expModal" class="fixed top-0 right-0 bottom-0 w-full sm:w-[420px] bg-white z-50 transform translate-x-full transition-transform duration-300 ease-out overflow-y-auto flex flex-col">
    ...
    <form id="expForm" class="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Tawi</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Tarehe *</label>
...
function openExpModal() {
    expEditMode = false;
    expEditId = null;
    document.getElementById('expModalTitle').innerHTML = '<div class="w-8 h-8 rounded-xl bg-emerald-50 grid place-items-center"><svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg></div> Matumizi Mapya';
...
function deleteExp(id) {
    Swal.fire({
        title: 'Una uhakika?', text: 'Matumizi haya yatafutwa kabisa.',
