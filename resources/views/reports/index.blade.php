...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Financial overview of your business — Year {{ now()->year }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('reports.sales') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-all">Sales</a>
            <a href="{{ route('reports.inventory') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-all">Inventory</a>
            <a href="{{ route('reports.profit-loss') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-all">Profit & Loss</a>
        </div>
    </div>
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-lg font-bold flex items-center gap-2">
                        ...
                        Annual Goals {{ now()->year }}
                    </h2>
                    <p class="text-emerald-200 text-xs mt-0.5">Track progress toward your financial goals</p>
                </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-white/5 rounded-xl p-4 border border-white/10 backdrop-blur-sm text-center">
                    ...
                    <p class="text-[10px] font-semibold text-emerald-200 uppercase">Sales</p>
                    <p class="text-sm font-bold mt-0.5">TZS {{ $fmt($yearSales) }}</p>
                    <p class="text-[10px] text-emerald-300/60">Goal: TZS {{ $fmt($annualSalesGoal) }}</p>
                </div>
                {{-- Profit Goal --}}
                <div class="bg-white/5 rounded-xl p-4 border border-white/10 backdrop-blur-sm text-center">
                    <div class="relative inline-flex items-center justify-center mb-3">
                    ...
                    <p class="text-[10px] font-semibold text-emerald-200 uppercase">Profit</p>
                    <p class="text-sm font-bold mt-0.5">TZS {{ $fmt($yearProfit) }}</p>
                    <p class="text-[10px] text-emerald-300/60">Goal: TZS {{ $fmt($annualProfitGoal) }}</p>
                </div>
                {{-- Expense Budget --}}
                <div class="bg-white/5 rounded-xl p-4 border border-white/10 backdrop-blur-sm text-center">
                    <div class="relative inline-flex items-center justify-center mb-3">
                    ...
                    <p class="text-[10px] font-semibold text-emerald-200 uppercase">Expenses</p>
                    <p class="text-sm font-bold mt-0.5">TZS {{ $fmt($yearExpenses) }}</p>
                    <p class="text-[10px] text-emerald-300/60">Budget: TZS {{ $fmt($annualExpenseBudget) }}</p>
                </div>
                {{-- Customer Goal --}}
                <div class="bg-white/5 rounded-xl p-4 border border-white/10 backdrop-blur-sm text-center">
                    <div class="relative inline-flex items-center justify-center mb-3">
                    ...
                    <p class="text-[10px] font-semibold text-emerald-200 uppercase">Customers</p>
                    <p class="text-sm font-bold mt-0.5">{{ $totalCustomers }}</p>
                    <p class="text-[10px] text-emerald-300/60">Goal: {{ round($annualCustomerGoal) }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-5 text-white shadow-sm relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-[10px] font-semibold text-emerald-100 uppercase">Today's Sales</span>
                </div>
                <p class="font-bold text-2xl">TZS {{ $fmt($todaySales) }}</p>
                <p class="text-[10px] text-emerald-200 mt-1">Profit: TZS {{ $fmt($todayProfit) }}</p>
            </div>
        </div>
        <div class="bg-gradient-to-br from-gold-400 to-gold-600 rounded-2xl p-5 text-white shadow-sm relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-[10px] font-semibold text-gold-100 uppercase">Weekly Sales</span>
                </div>
                <p class="font-bold text-2xl">TZS {{ $fmt($weekSales) }}</p>
                <p class="text-[10px] text-gold-200 mt-1">Month: TZS {{ $fmt($monthSales) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-200 relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-[10px] font-semibold text-gray-400 uppercase">Yearly Sales</span>
                </div>
                <p class="font-bold text-2xl text-emerald-600">TZS {{ $fmt($yearSales) }}</p>
                <p class="text-[10px] text-gray-400 mt-1">Profit: TZS {{ $fmt($yearProfit) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-200 relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-[10px] font-semibold text-gray-400 uppercase">Yearly Expenses</span>
                </div>
                <p class="font-bold text-2xl text-red-500">TZS {{ $fmt($yearExpenses) }}</p>
                <p class="text-[10px] text-gray-400 mt-1">Income: TZS {{ $fmt($yearIncome) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-xl border p-3 flex items-center gap-3">
            ...
            <div><p class="text-[10px] font-semibold text-gray-400 uppercase">Monthly Profit</p><p class="font-bold text-sm text-emerald-600">TZS {{ $fmt($monthProfit) }}</p></div>
        </div>
        <div class="bg-white rounded-xl border p-3 flex items-center gap-3">
            ...
            <div><p class="text-[10px] font-semibold text-gray-400 uppercase">Monthly Expenses</p><p class="font-bold text-sm text-red-500">TZS {{ $fmt($monthExpenses) }}</p></div>
        </div>
        <div class="bg-white rounded-xl border p-3 flex items-center gap-3">
            ...
            <div><p class="text-[10px] font-semibold text-gray-400 uppercase">Total Debts</p><p class="font-bold text-sm text-gold-600">TZS {{ $fmt($totalDebts) }}</p></div>
        </div>
        <div class="bg-white rounded-xl border p-3 flex items-center gap-3">
            ...
            <div><p class="text-[10px] font-semibold text-gray-400 uppercase">Overdue Debts</p><p class="font-bold text-sm text-red-500">TZS {{ $fmt($overdueDebts) }}</p></div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex gap-2 text-[10px]">
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-600"></span>Sales</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-gold-500"></span>Profit</span>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-gray-800 flex items-center gap-2">
                    ...
                    Sales vs Expenses vs Profit (12 Months)
                </h3>
                <div class="flex gap-2 text-[10px]">
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-600"></span>Sales</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-400"></span>Expenses</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-gold-500"></span>Profit</span>
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <span class="text-xs font-bold text-emerald-600">{{ $p->total_qty }} sold</span>
                    </div>
                ...
                @empty
                <p class="text-gray-400 font-medium text-center py-8">No data.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <div class="flex-1">
                        <p class="font-semibold text-sm text-gray-700">{{ $c->customer?->name ?? 'Customer' }}</p>
                        <p class="text-[10px] text-gray-400">{{ $c->orders_count }} orders</p>
                    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            @empty
            <div class="col-span-full text-center py-8 text-sm text-gray-400">All stock levels are good!</div>
            @endforelse
        </div>
    </div>
</div>
