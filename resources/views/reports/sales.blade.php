...
<div class="space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Filter and view all sales</p>
        </div>
    </div>
    ...
    <form method="GET" class="flex gap-2 sm:gap-3 flex-wrap bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <select name="branch" class="px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="">All Branches</option>@foreach($branches as $b)<option value="{{ $b->id }}" @selected(request('branch') == $b->id)>{{ $b->name }}</option>@endforeach</select>
        <select name="method" class="px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none text-sm font-medium bg-white appearance-none cursor-pointer transition-all"><option value="">All Methods</option><option value="cash">Cash</option><option value="mpesa">M-Pesa</option><option value="tigo_pesa">Tigo Pesa</option><option value="airtel_money">Airtel Money</option><option value="credit">Credit</option></select>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-500 font-semibold text-xs uppercase tracking-wide">
                    <tr><th class="px-4 py-3 text-left">Receipt</th><th class="px-4 py-3 text-left hidden sm:table-cell">Branch</th><th class="px-4 py-3 text-left hidden md:table-cell">Salesperson</th><th class="px-4 py-3 text-left hidden md:table-cell">Customer</th><th class="px-4 py-3 text-center">Method</th><th class="px-4 py-3 text-right">Total</th><th class="px-4 py-3 text-left hidden sm:table-cell">Date</th></tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-600 hidden md:table-cell">{{ $s->customer?->name ?? 'Walk-in' }}</td>
                    ...
                    <tr><td colspan="7" class="px-4 py-16 text-center">
                        ...
                        <p class="text-gray-400 font-medium text-sm">No sales found.</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
